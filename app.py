...
from flask_login import LoginManager, login_user, login_required  # noqa: E402
    ...
    if request.method == 'POST':
        ...
        if username and password is not '':
            ...
            if user.verify_password(password) and user is not '':
                ...
                # setting the user logged in session
...
@app.route('/')
def index():
    """This function fetch all the categories from the catalog and render the
    homepage"""
...
def getUserID(email):
    """This function fetch a user from the email passed and return that user's
    id."""
    try:
...
def googleConnect():
    ...
    try:
        ...
        # exchanging the code for user credentials
...
def createCategory():
    ...
    if request.method == 'POST':
        ...
        if inputtedName is not '':
            ...
            # fetching a single category name from the db and storing it in an
            # object
            fetchedCategoryName = session.query(
                Category).filter_by(name=inputtedName).first()
            ...
            if fetchedCategoryName is None:
                ...
                if fetchedCategoryName.name == inputtedName:
                    flash(
                        'Sorry \'{}\' category is already'
                        'existing'.format(inputtedName))
        else:
...
def allCategories():
    """This function fetch and return all the categories a user
    have created."""
...
def editCategory(categoryName):
    ...
    fetchedCategoryName = session.query(
        Category).filter_by(name=categoryName).first()
...
def deleteCategory(categoryName):
    """This function give previledge to logged in users to delete only
    categories the created."""

    # fetching a single category from the db and storing it in an object
    fetchedCategoryName = session.query(
        Category).filter_by(name=categoryName).first()
    categoryToDel = session.query(Category).filter_by(
        id=fetchedCategoryName.id).one()
...
def allItems(categoryName):
    """This function fetch all items from a category and return it to the items
    template."""
...
def createItem(categoryName):
    ...
    if request.method == 'POST':
        ...
        if itemName and itemDescription is not '':
            ...
            # fetching a single item name from the db and storing it in an
            # object
            ...
            if fetchedItem is None:
                ...
                flash(
                    'The Item \'{}\' is already existing'.format(
                        fetchedItem.name))
        else:
